...
class LinearQuadraticRegulator(Controller):
    ...
    def control(
        ...
        pred_x_sequence = self._transition_model.predict_trajectory(
            curr_x, initial_u_sequence, random_key
        )
        # without batch
        Sk = self._cost_function.terminal_cxx(
            pred_x_sequence[self._T][jnp.newaxis, :], self._T
        )[0]
        fx_random_keys = jax.random.split(random_key, num=self._T)
        fu_random_keys = jax.random.split(random_key, num=self._T)
        ...
        random_keys = jax.random.split(random_key)
        x = curr_x
        for t, K in enumerate(reversed(gains)):
            ...
            x = self._transition_model.predict_next_state(
                x, optimized_u, t, random_keys[t]
            )
