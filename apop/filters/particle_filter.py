...
class ParticleFilter(Filter):
    ...
    def estimate(self, curr_y: jnp.ndarray, mask: jnp.ndarray) -> jnp.ndarray:
        ...
        self._x_particles, self._particle_weights = self._filter(
            self._x_particles, self._particle_weights, curr_y, mask
        )
        return jnp.sum(
            self._particle_weights[:, jnp.newaxis] * self._x_particles, axis=0
        )
    ...
    def _filter(
        ...
        # filtering step
        # computes Nff
        Nff = self._num_particles / jnp.sum(particle_weights**2)
        # resample
        ...
        new_x_particles, updated_particle_weights = jax.lax.cond(
            ...
            self._resample,
            lambda particle, weights: (particle, weights),
            x_particles,
            particle_weights,
        )
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _resample(self, x_particles: jnp.ndarray, particle_weights: jnp.ndarray):
